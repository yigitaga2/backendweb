    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('admin.news.update', $news) }}" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" :value="old('title', $news->title)" required autofocus placeholder="Enter an engaging news title" />
                        ...
                        <div>
                            ...
                            <x-text-input id="slug" name="slug" type="text" class="mt-1 block w-full" :value="old('slug', $news->slug)" required placeholder="url-friendly-slug" />
                        ...
                        @if($news->featured_image)
                            <div>
                                ...
                                <div class="mt-2">
                                    <img src="{{ $news->featured_image_url }}" alt="Current featured image" class="max-w-xs rounded-lg shadow-md">
                                </div>
                            </div>
                        @endif
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-600">Upload a new featured image to replace the current one (JPG, PNG, GIF, max 2MB).</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="flex items-center">
                                    <input type="checkbox" name="is_published" value="1" {{ old('is_published', $news->is_published) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
            ...
            <div class="mt-6 bg-red-50 rounded-lg p-6">
                ...
                <p class="text-red-800 text-sm mb-4">Once you delete this news article, it cannot be recovered.</p>
                <form method="POST" action="{{ route('admin.news.destroy', $news) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this news article? This action cannot be undone.')">
                    @csrf
                    @method('DELETE')
